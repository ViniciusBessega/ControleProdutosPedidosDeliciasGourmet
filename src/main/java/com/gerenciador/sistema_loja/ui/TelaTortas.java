...
public class TelaTortas {
    ...
    public TelaTortas(StackPane rootPrincipal, ProdutoService service, CarrinhoService carrinhoService) {
    ...
    public Parent criarTela() {
        ...
        Label titulo = new Label("Tortas");
        titulo.setStyle("-fx-font-size: 22px; -fx-font-weight: bold;");

        TextField campoBusca = new TextField();
        campoBusca.setPromptText("Buscar torta...");
    ...
    private void atualizarLista(VBox lista, String busca, Label totalLabel) {
        ...
        List<Torta> tortas = service.listarTortas();

        if (busca != null && !busca.isBlank()) {
            String b = busca.toLowerCase();
            tortas = tortas.stream()
                    .filter(t -> t.getNome().toLowerCase().contains(b))
                    .toList();
        }

        for (Torta t : tortas) {

            Label nome = new Label(t.getNome());
